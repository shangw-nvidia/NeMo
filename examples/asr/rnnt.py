...
def parse_args():
    ...
    # Create new args
    parser.add_argument("--exp_name", default="RNNT", type=str)
    ...
    if args.max_steps is not None:
        raise ValueError("RNNT uses num_epochs instead of max_steps")
...
def create_all_dags(args, neural_factory):
    ...
    with open(args.model_config) as f:
        rnnt_params = yaml.load(f)

    vocab = rnnt_params['labels']
    sample_rate = rnnt_params['sample_rate']
    ...
    train_dl_params = copy.deepcopy(rnnt_params["AudioToTextDataLayer"])
    train_dl_params.update(rnnt_params["AudioToTextDataLayer"]["train"])
    ...
    eval_dl_params = copy.deepcopy(rnnt_params["AudioToTextDataLayer"])
    eval_dl_params.update(rnnt_params["AudioToTextDataLayer"]["eval"])
    ...
    data_preprocessor = nemo_asr.AudioToMelSpectrogramPreprocessor(
        sample_rate=sample_rate, **rnnt_params["AudioToMelSpectrogramPreprocessor"],
    )

    # Inject the `kernel_size_factor` kwarg to the RNNT config
    # Skip the last layer  as that must be a pointwise kernel
    # for idx in range(len(rnnt_params["ContextNetEncoder"]["jasper"]) - 1):
    #     rnnt_params["ContextNetEncoder"]["jasper"][idx]["kernel_size_factor"] = args.kernel_size_factor

    # (ContextNet uses the Jasper baseline encoder and decoder)
    encoder = nemo_asr.RNNTEncoder(
        feat_in=rnnt_params["AudioToMelSpectrogramPreprocessor"]["features"], **rnnt_params["RNNTEncoder"],
    )

    decoder = nemo_asr.RNNTDecoder(num_classes=len(vocab), **rnnt_params["RNNTDecoder"])

    joint = nemo_asr.RNNTJoint(num_classes=len(vocab), **rnnt_params['RNNTJoint'])
    ...
    spectr_augment_config = rnnt_params.get('SpectrogramAugmentation', None)
